...
The client implementation must be written in TypeScript against the Zero `Transaction` interface, using [ZQL](#reading-in-client-mutators) for reads and a [CRUD-style API](#writing-in-client-mutators) for writes.
...
export function createMutators() {
  return {
    issue: {
      update: (tx, {id, title}: { id: string; title: string }) => {
...
export function createMutators(authData: AuthData|undefined) {
  return {
    issue: {
      launchMissiles: (tx, args: { target: string }) => {
        if (!authData) {
...
processor.process(
  createMutators(decodedJWT), c.req.query(), c.req.json());
```
